...
from .models import BalanceAdjustment, Bucket, Amount, ValidFor, ProductRef
...
class BalanceManagement:
    ...
    def create_adjustment(
        ...
        Example:
            ...
            >>> result = balance.create_adjustment("1605455656771-64", 1000, "cents")
        """
        data = {
            "amount": {
                "units": units,
    ...
    def list_buckets(self, product_id: str, parse: bool = True) -> List[Dict[str, Any]]:
        """
        ...
        Returns:
            List of bucket data (parsed or raw based on parse parameter)
        """
        raw_buckets = self.client.get(f"{self.base_endpoint}/bucket", params={"product.id": product_id})
        
        if parse:
            return [self._parse_bucket_data(bucket) for bucket in raw_buckets]
        else:
            return raw_buckets
